...
function l.dec_file(f) if isfile(f) then return l.decode(readfile(f)) end end;
function l.decsave_file(f) if isfile(f) then wf(f, dec_file(f)) end end;
function l.save_table(n, tbl) wf_enc(n, tbl) if isfile(n) then l.decsave_file(n) end end;
